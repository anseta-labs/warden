# Anseta Warden

Warden is a transaction validation library for ensuring that transactions generated by integrations with Anseta services. It follows zero-trust methodologies while ensuring transaction integrity prior to signing.

Anseta Warden is built to protect customers from malicious transactions and is built to ensure that all transactions derived from the Anseta API, SDK's, and other integrations remain untampered from source to broadcast.

Likewise, Warden includes a number of helpful integrations with other Anseta utilities.

# @anseta/warden

TypeScript library for **Anseta integrators**: validate **unsigned** transactions (e.g. EVM hex) **before** signing, using rules keyed by `chainId` and `TransactionType`.

## Supported Chains

- Ethereum
- Solana

## Requirements

- Node **20+**
- **pnpm**

## Setup

```bash
pnpm install
pnpm run build
pnpm test
```

## Usage

### General Transction Usage

```ts
import {
  Warden,
  TransactionType,
  type ValidationRequest,
} from '@anseta/warden';

const warden = new Warden();

const chains = warden.getSupportedChains(); // e.g. [1, 560048]
warden.getSupportedTransactionTypes(chains[0]); // e.g. ['DEPOSIT', 'FORCE_EXIT', 'WITHDRAW']
warden.isSupported(chains[0], TransactionType.DEPOSIT); // true if registered
warden.isSupported(1, TransactionType.WITHDRAW); // EIP-7002 partial withdrawal
warden.isSupported(1, TransactionType.FORCE_EXIT); // EIP-7002 full-exit (0 gwei in calldata)

// Unsigned EIP-1559 (or legacy) tx hex from your API — do not sign before validating
const request: ValidationRequest = {
  unsignedTransaction: '0x02f8...', // RLP-serialized unsigned tx, 0x-prefixed
  userAddress: '0x...',
  chainId: chains[0],
  transactionType: TransactionType.DEPOSIT,
  // args: optional integrator hints; context: optional; Warden merges chainId into context for validators
};

const result = warden.validate(request);
if (result.isValid) {
  // Safe to present to the wallet for signing
  console.log(result.detectedType); // same as request.transactionType when valid
} else {
  // Request validation failed, do not proceed with signing!
  console.error(result.reason, result.details);
}
```

### Anseta API Usage

```ts
import {
  Warden,
  TransactionType,
  type ValidationRequest,
} from '@anseta/warden';

const warden = new Warden();
const chains = warden.getSupportedChains();

// Anseta Ethereum Staking Transaction creation endpoint
const ansetaStakeEthTxEndpoint =
  'https://preview.api.stakefi.network/v1/ethereum-staking/stake';

const options = {
  method: 'POST',
  headers: {
    'x-api-key': process.env.ANSETA_API_KEY,
  },
  body: {
    network: 'ethereum-mainnet',
    amountWei: '64000000000000000000',
    withdrawalAddress: '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045',
    feeRecipient: '0x388C818CA8B9251b393131C08a736A67ccB19297',
    validatorName: 'my-validator-mainnet-01',
    gasLimit: 36000000,
  },
};

const apiResp = await fetch(ansetaStakeEthTxEndpoint, options);

// Skipping Fetch Request Checks for Illustration purposes
// in Production, along with standard HTTP request checks, check for
// apiResp.success checks as well

const { data } = apiResp;

const request: ValidationRequest = {
  unsignedTransaction: data.depositTransaction.encodedTx,
  userAddress: '0x...',
  chainId: chains[0],
  transactionType: TransactionType.DEPOSIT,
};

result = warden.validate(request);

if (result.isValid) {
  // Safe to present to the wallet for signing
  console.log(result.detectedType); // same as request.transactionType when valid
} else {
  // Request validation failed, do not proceed with signing!
  console.error(result.reason, result.details);
}
```

## Examples

Runnable scripts under **`examples/`** (from the repo root, use `pnpm exec ts-node examples/<file>.ts`):

| File                                    | What it covers                                                                                            |
| --------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `01_basic_eth2_deposit.ts`              | **`DEPOSIT`**: valid beacon deposit vs withdrawal-credentials mismatch                                    |
| `02_eip7002_partial_and_full_exit.ts`   | **`WITHDRAW`** and **`FORCE_EXIT`**: EIP-7002 predeploy, partial amount vs full-exit (0 gwei in calldata) |
| `03_solana_stake.ts`                    | **`STAKE`**: Solana native stake (CreateAccountWithSeed → Initialize → Delegate)                          |
| `04_solana_partial_and_full_unstake.ts` | **`UNSTAKE`**: partial (Split + Deactivate) vs full (Deactivate only)                                     |

```bash
pnpm exec ts-node examples/01_basic_eth2_deposit.ts
pnpm exec ts-node examples/script-name-here.ts
```

## Scripts

| Script                            | Purpose                                                                                                      |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `pnpm run build`                  | Compile to `dist/`                                                                                           |
| `pnpm test`                       | Jest (verbose per-test output is enabled in `jest.config.mjs`)                                               |
| `pnpm run generate-eth2-fixtures` | Regenerate **`src/validators/evm/__fixtures__/eth2-deposit-unsigned-hex.json`** used by beacon deposit tests |

## Test fixtures

`EthereumBeaconValidator` tests use **frozen unsigned tx hex** in `__fixtures__/`. That avoids pulling **`@noble/curves`** (ESM BLS used only in `build-eth2-tx`) into Jest, which is easier than ESM-only test runner setup. After changing deposit building logic, run **`pnpm run generate-eth2-fixtures`** and commit the updated JSON.
